(function(ext) {
	...
	const BLOCKS = {
		...
		ja2: [
			...
			["w", "%m.left_right 車輪を中心に %n %m.deg_sec %m.forward_backward 方向に回す", "albertaiPivotAroundWheelUnitInDirection", "左", 90, "度", "前"],
			["-"],
			[" ", "%m.left_right_both 眼を %m.led_color にする", "albertaiSetEyeTo", "左", "赤色"],
			[" ", "%m.left_right_both 眼を消す", "albertaiClearEye", "左"],
			["-"],
		...
		ja3: [
			...
			["w", "%m.left_right 車輪を中心に %n %m.turn_unit %m.forward_backward 方向に回す", "albertaiPivotAroundWheelUnitInDirection", "左", 90, "度", "前"],
			...
			[" ", "停止する", "albertaiStop"],
			["w", "ボード板上で %m.move_forward_backward x: %n y: %n 位置に移動する", "albertaiMoveToOnBoard", "前へ", 0, 0],
			["w", "ボード板上で %n 度に向ける", "albertaiSetOrientationToOnBoard", 0],
			["-"],
			[" ", "%m.left_right_both 眼を %m.led_color にする", "albertaiSetEyeTo", "左", "赤色"],
			...
			["w", "%m.note %m.octave 音を %n 拍鳴らす", "albertaiPlayNoteFor", "ド", "4", 0.5],
			["w", "%n 拍休む", "albertaiRestFor", 0.25],
			[" ", "テンポを %n ずつ変える", "albertaiChangeTempoBy", 20],
			[" ", "テンポを %n BPMにする", "albertaiSetTempoTo", 60],
			["-"],
