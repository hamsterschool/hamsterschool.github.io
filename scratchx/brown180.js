(function(ext) {
	...
	const BLOCKS = {
		...
		ja1: [
			...
			["w", "%m.left_right へ回る", "brownTurn", "左"],
			["-"],
			[" ", "LEDを %m.led_color にする", "brownSetLedTo", "赤色"],
			[" ", "LEDをオフにする", "brownClearLed"],
			["-"],
			[" ", "%m.sound_effect 音を再生する", "brownPlaySound", "ビープ"],
			[" ", "音をオフにする", "brownClearSound"],
			["-"],
		...
		ja2: [
			...
			["w", "%m.left_right へ %n %m.deg_sec その場所で回る", "brownTurnUnitInPlace", "左", 90, "度"],
			["w", "%m.left_right 車輪を中心に %n %m.deg_sec %m.forward_backward 方向へ回る", "brownPivotAroundWheelUnitInDirection", "左", 90, "度", "前"],
			["w", "%m.left_right へ %n %m.deg_sec 半径 %n cmを %m.forward_backward 方向へ回る", "brownTurnUnitWithRadiusInDirection", "左", 90, "度", 6, "前"],
			["-"],
			[" ", "LEDを %m.led_color にする", "brownSetLedTo", "赤色"],
			[" ", "LEDをオフにする", "brownClearLed"],
			["-"],
			[" ", "%m.sound_effect 音を %n 回再生する", "brownPlaySoundTimes", "ビープ", 1],
			["w", "%m.sound_effect 音を %n 回再生して待つ", "brownPlaySoundTimesUntilDone", "ビープ", 1],
			[" ", "音をオフにする", "brownClearSound"],
			["w", "%m.note %m.octave 音を %d.beats 拍子奏でる", "brownPlayNoteForBeats", "ド", "4", 0.5],
			["w", "%d.beats 拍子止める", "brownRestForBeats", 0.25],
			[" ", "演奏の速さを %n ずつ変える", "brownChangeTempoBy", 20],
			[" ", "演奏の速さを %n BPMにする", "brownSetTempoTo", 60],
			["-"],
			["h", "%m.touching_color に触れたとき", "brownWhenColorTouched", "赤色"],
		...
		ja3: [
			...
			["w", "%m.left_right へ %n %m.turn_unit その場所で回る", "brownTurnUnitInPlace", "左", 90, "度"],
			["w", "%m.left_right 車輪を中心に %n %m.turn_unit %m.forward_backward 方向へ回る", "brownPivotAroundWheelUnitInDirection", "左", 90, "度", "前"],
			["w", "%m.left_right へ %n %m.turn_unit 半径 %n cmを %m.forward_backward 方向へ回る", "brownTurnUnitWithRadiusInDirection", "左", 90, "度", 6, "前"],
			...
			[" ", "線に沿って移動する", "brownFollowLine"],
			["w", "線に沿って %m.target_color まで移動する", "brownFollowLineUntil", "赤色"],
			["w", "交差点を渡る", "brownCrossIntersection"],
			["w", "交差点で %m.left_right_back へ回る", "brownTurnAtIntersection", "左"],
			["w", "%m.left_right 線へ渡り行く", "brownJumpLine", "左"],
			[" ", "線に沿って移動する速さを %m.speed にする", "brownSetFollowingSpeedTo", "4"],
			[" ", "停止する", "brownStop"],
			["-"],
			[" ", "LEDを %m.led_color にする", "brownSetLedTo", "赤色"],
			...
			[" ", "音をオフにする", "brownClearSound"],
			[" ", "%m.note %m.octave 音を奏でる", "brownPlayNote", "ド", "4"],
			["w", "%m.note %m.octave 音を %d.beats 拍子奏でる", "brownPlayNoteForBeats", "ド", "4", 0.5],
			["w", "%d.beats 拍子止める", "brownRestForBeats", 0.25],
			[" ", "演奏の速さを %n ずつ変える", "brownChangeTempoBy", 20],
			[" ", "演奏の速さを %n BPMにする", "brownSetTempoTo", 60],
			...
			["b", "ボタンを %m.button_state ?", "brownButtonState", "クリックしたか"],
			["b", "%m.s_tilt ?", "brownTilt", "前に傾けたか"],
			["b", "電池充電が %m.battery ?", "brownBattery", "正常か"]
		],
		uz1: [
