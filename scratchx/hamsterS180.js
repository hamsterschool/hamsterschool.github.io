(function(ext) {
	...
	const BLOCKS = {
		...
		ja1: [
			["w", "ボード板上で前へ移動する", "sBoardMoveForward"],
			["w", "ボード板上で %m.left_right へ回る", "sBoardTurn", "左"],
			["-"],
			["w", "前へ移動する", "sMoveForward"],
			["w", "後ろへ移動する", "sMoveBackward"],
			["w", "%m.left_right へ回る", "sTurn", "左"],
			["-"],
			[" ", "%m.left_right_both LEDを %m.led_color にする", "sSetLedTo", "左", "赤色"],
			[" ", "%m.left_right_both LEDをオフにする", "sClearLed", "左"],
			["-"],
			[" ", "%m.sound_effect 音を再生する", "sPlaySound", "ビープ"],
			[" ", "音をオフにする", "sClearSound"],
			["-"],
		...
		ja2: [
			...
			["w", "ボード板上で %m.left_right へ回る", "sBoardTurn", "左"],
			["-"],
			["w", "前へ %n %m.cm_sec 移動する", "sMoveForwardUnit", 5, "cm"],
			["w", "後ろへ %n %m.cm_sec 移動する", "sMoveBackwardUnit", 5, "cm"],
			["w", "%m.left_right へ %n %m.deg_sec その場所で回る", "sTurnUnitInPlace", "左", 90, "度"],
			["w", "%m.left_right 車輪を中心に %n %m.deg_sec %m.forward_backward 方向へ回る", "sPivotAroundWheelUnitInDirection", "左", 90, "度", "前"],
			["w", "%m.left_right へ %n %m.deg_sec 半径 %n cmを %m.forward_backward 方向へ回る", "sTurnBodyUnitWithRadiusInDirection", "左", 90, "度", 5, "前"],
			["-"],
			[" ", "%m.left_right_both LEDを %m.led_color にする", "sSetLedTo", "左", "赤色"],
			[" ", "%m.left_right_both LEDをオフにする", "sClearLed", "左"],
			["-"],
			[" ", "%m.sound_effect 音を %n 回再生する", "sPlaySoundTimes", "ビープ", 1],
			["w", "%m.sound_effect 音を %n 回再生して待つ", "sPlaySoundTimesUntilDone", "ビープ", 1],
			[" ", "音をオフにする", "sClearSound"],
			["w", "%m.note %m.octave 音を %d.beats 拍子奏でる", "sPlayNoteFor", "ド", "4", 0.5],
			["w", "%d.beats 拍子止める", "sRestFor", 0.25],
			[" ", "演奏の速さを %n ずつ変える", "sChangeTempoBy", 20],
			[" ", "演奏の速さを %n BPMにする", "sSetTempoTo", 60],
			["-"],
			["h", "手を見つけたとき", "sWhenHandFound"],
		...
		ja3: [
			...
			["w", "ボード板上で %m.left_right へ回る", "sBoardTurn", "左"],
			["-"],
			["w", "前へ %n %m.move_unit 移動する", "sMoveForwardUnit", 5, "cm"],
			["w", "後ろへ %n %m.move_unit 移動する", "sMoveBackwardUnit", 5, "cm"],
			["w", "%m.left_right へ %n %m.turn_unit その場所で回る", "sTurnUnitInPlace", "左", 90, "度"],
			["w", "%m.pen_wheel を中心に %n %m.turn_unit %m.forward_backward 方向へ回る", "sPivotAroundUnitInDirection", "左ペン", 90, "度", "前"],
			["w", "%m.pen_body %m.left_right へ %n %m.turn_unit 半径 %n cmを %m.forward_backward 方向へ回る", "sTurnUnitWithRadiusInDirection", "左ペン", "左", 90, "度", 5, "前"],
			...
			[" ", "%m.black_white 線に沿って %m.left_right_both 床底センサーで移動する", "sFollowLineUsingFloorSensor", "黒色", "左"],
			["w", "%m.black_white 線に沿って %m.left_right_front_rear 交差点まで移動する", "sFollowLineUntilIntersection", "黒色", "前"],
			[" ", "線に沿って移動する速さを %m.speed にする", "sSetFollowingSpeedTo", "5"],
			[" ", "線に沿って移動する方向変化量を %m.gain にする", "sSetFollowingGainTo", "基本値"],
			[" ", "停止する", "sStop"],
			["-"],
			[" ", "%m.left_right_both LEDを %m.led_color にする", "sSetLedTo", "左", "赤色"],
			...
			[" ", "音をオフにする", "sClearSound"],
			[" ", "%m.note %m.octave 音を奏でる", "sPlayNote", "ド", "4"],
			["w", "%m.note %m.octave 音を %d.beats 拍子奏でる", "sPlayNoteFor", "ド", "4", 0.5],
			["w", "%d.beats 拍子止める", "sRestFor", 0.25],
			[" ", "演奏の速さを %n ずつ変える", "sChangeTempoBy", 20],
			[" ", "演奏の速さを %n BPMにする", "sSetTempoTo", 60],
			...
			["r", "左床底センサー", "sLeftFloor"],
			["r", "右床底センサー", "sRightFloor"],
			...
			["b", "手を見つけたか?", "sHandFound"],
			["b", "%m.s_tilt ?", "sTilt", "前に傾けたか"],
			["b", "電池充電が %m.battery ?", "sBattery", "正常か"],
			["-"],
		...
		uz3: [
			...
			[" ", "ketma-ket tezligini %m.serial_baud Bd ga sozlash", "sSetSerialRateTo", "9600"],
	...
	ext.sTurnBodyUnitWithRadiusInDirection = function(direction, value, unit, radius, toward, callback) {
		var robot = getRobot(HAMSTER, 0);
		if(robot) robot.swingBodyUnit(direction, value, unit, radius, toward, callback);
	};
	
	ext.sPivotAroundUnitInDirection = function(part, value, unit, toward, callback) {
		var robot = getRobot(HAMSTER, 0);
		if(robot) robot.pivotUnit(part, value, unit, toward, callback);
	};
	
	ext.sTurnUnitWithRadiusInDirection = function(part, direction, value, unit, radius, toward, callback) {
		var robot = getRobot(HAMSTER, 0);
