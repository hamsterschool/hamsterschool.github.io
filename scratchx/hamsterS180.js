(function(ext) {
	...
	const BLOCKS = {
		...
		ja1: [
			...
			["w", "ボード板上で %m.left_right へ回す", "sBoardTurn", "左"],
			["-"],
			["w", "前へ移動する", "sMoveForward"],
			["w", "後ろへ移動する", "sMoveBackward"],
			["w", "%m.left_right へ回す", "sTurn", "左"],
			["-"],
			[" ", "%m.left_right_both LEDを %m.led_color にする", "sSetLedTo", "左", "赤色"],
			[" ", "%m.left_right_both LEDを消す", "sClearLed", "左"],
			["-"],
		...
		ja2: [
			...
			["w", "%m.left_right へ %n %m.deg_sec その場で回す", "sTurnUnitInPlace", "左", 90, "度"],
			["w", "%m.left_right 車輪を中心に %n %m.deg_sec %m.forward_backward 方向へ回す", "sPivotAroundWheelUnitInDirection", "左", 90, "度", "前"],
			["w", "%m.left_right へ %n %m.deg_sec 半径 %n cmを %m.forward_backward 方向へ回す", "sTurnBodyUnitWithRadiusInDirection", "左", 90, "度", 5, "前"],
			["-"],
			[" ", "%m.left_right_both LEDを %m.led_color にする", "sSetLedTo", "左", "赤色"],
			[" ", "%m.left_right_both LEDを消す", "sClearLed", "左"],
			["-"],
		...
		ja3: [
			...
			["w", "%m.left_right へ %n %m.turn_unit その場で回す", "sTurnUnitInPlace", "左", 90, "度"],
			["w", "%m.pen_wheel を中心に %n %m.turn_unit %m.forward_backward 方向へ回す", "sPivotAroundUnitInDirection", "左ペン", 90, "度", "前"],
			["w", "%m.pen_body %m.left_right へ %n %m.turn_unit 半径 %n cmを %m.forward_backward 方向へ回す", "sTurnUnitWithRadiusInDirection", "左ペン", "左", 90, "度", 5, "前"],
			...
			["w", "%m.black_white 線を追従して %m.left_right_front_rear 交差点まで移動する", "sFollowLineUntilIntersection", "黒色", "前"],
