(function(ext) {
	...
	const BLOCKS = {
		...
		ja1: [
			...
			["w", "%m.left_right へ回る", "zeroneTurn", "左"],
			["-"],
			[" ", "%m.left_right_head_tail_all LEDを %m.led_color にする", "zeroneSetLedTo", "前", "赤色"],
			[" ", "%m.left_right_head_tail_all LEDをオフにする", "zeroneClearLed", "前"],
			["-"],
			[" ", "%m.sound_effect 音を再生する", "zeronePlaySound", "ビープ"],
			[" ", "音をオフにする", "zeroneClearSound"],
			["-"],
		...
		ja2: [
			...
			["w", "%m.left_right へ %n %m.deg_sec その場所で回る", "zeroneTurnUnitInPlace", "左", 90, "度"],
			["-"],
			[" ", "%m.left_right_head_tail_all LEDを %m.led_color にする", "zeroneSetLedTo", "前", "赤色"],
			[" ", "%m.left_right_head_tail_all LEDをオフにする", "zeroneClearLed", "前"],
			["-"],
			[" ", "%m.sound_effect 音を %n 回再生する", "zeronePlaySoundTimes", "ビープ", 1],
			["w", "%m.sound_effect 音を %n 回再生して待つ", "zeronePlaySoundTimesUntilDone", "ビープ", 1],
			[" ", "音をオフにする", "zeroneClearSound"],
			["w", "%m.note %m.octave 音を %d.beats 拍子奏でる", "zeronePlayNoteFor", "ド", "4", 0.5],
			["w", "%d.beats 拍子止める", "zeroneRestFor", 0.25],
			[" ", "演奏の速さを %n ずつ変える", "zeroneChangeTempoBy", 20],
			[" ", "演奏の速さを %n BPMにする", "zeroneSetTempoTo", 60],
			["-"],
			[" ", "%m.zerone_sensor_mode センサーを開始する", "zeroneStartSensor", "ジェスチャー"],
			["h", "ジェスチャーが %m.gesture であるとき", "zeroneWhenGesture", "前へ"],
			["h", "%m.zerone_color に触れたとき", "zeroneWhenColorTouched", "赤色"],
		...
		ja3: [
			...
			["w", "%m.left_right へ %n %m.turn_unit その場所で回る", "zeroneTurnUnitInPlace", "左", 90, "度"],
			...
			[" ", "音をオフにする", "zeroneClearSound"],
			[" ", "%m.note %m.octave 音を奏でる", "zeronePlayNote", "ド", "4"],
			["w", "%m.note %m.octave 音を %d.beats 拍子奏でる", "zeronePlayNoteFor", "ド", "4", 0.5],
			["w", "%d.beats 拍子止める", "zeroneRestFor", 0.25],
			[" ", "演奏の速さを %n ずつ変える", "zeroneChangeTempoBy", 20],
			[" ", "演奏の速さを %n BPMにする", "zeroneSetTempoTo", 60],
			...
			["b", "ジェスチャーが %m.gesture ですか?", "zeroneIsGesture", "前へ"],
			["b", "%m.zerone_color に触れたか?", "zeroneTouchingColor", "赤色"],
			["b", "電池充電が %m.battery ?", "zeroneBattery", "正常か"]
		],
		uz1: [
