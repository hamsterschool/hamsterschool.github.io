(function(ext) {
	...
	const BLOCKS = {
		...
		ja1: [
			...
			["w", "%m.left_right へ回す", "turn", "左"],
			["-"],
			[" ", "%m.left_right_both 眼を %m.color にする", "setEyeTo", "左", "赤色"],
			[" ", "%m.left_right_both 眼を消す", "clearEye", "左"],
			[" ", "胴体LEDを %m.on_off", "turnBodyLed", "点灯"],
			[" ", "前方LEDを %m.on_off", "turnFrontLed", "点灯"],
			["-"],
			["w", "ビープ", "beep"],
			["-"],
		...
		ja2: [
			...
			["w", "%m.left_right へ %n 秒回す", "turnForSecs", "左", 1],
			["-"],
			[" ", "%m.left_right_both 眼を %m.color にする", "setEyeTo", "左", "赤色"],
			[" ", "%m.left_right_both 眼を消す", "clearEye", "左"],
			[" ", "胴体LEDを %m.on_off", "turnBodyLed", "点灯"],
		...
		ja3: [
			...
			["w", "ボード板上で %m.move_forward_backward x: %n y: %n 位置へ移動する", "moveToOnBoard", "前へ", 0, 0],
			["w", "ボード板上で %n 度へ向ける", "setOrientationToOnBoard", 0],
			["-"],
			[" ", "%m.left_right_both 眼を %m.color にする", "setEyeTo", "左", "赤色"],
			[" ", "%m.left_right_both 眼を消す", "clearEye", "左"],
			[" ", "胴体LEDを %m.on_off", "turnBodyLed", "点灯"],
			...
			try {
				...
				sock.onmessage = function(message) {
					try {
						...
						if(received.type == 0) {
							if(received.states) {
								connectionState = received.states[ALBERTSCHOOL];
							}
						} else {
							if(received.index >= 0) {
